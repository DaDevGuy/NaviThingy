<script lang="ts">
	...
	import type { EnhancedSyncedLyric, SyncedLyric, SyncedWord } from '$lib/types/navidrome';
	import {
		Heart,
		ListMusic,
		Maximize2,
	...
	$effect(() => {
		if ($player.currentTrack) {
			...
			if (
				...
				(async () => {
					const lyricsSource = localStorage.getItem('lyricsSource') || 'external';
					let lyricsResult = null;

					if (lyricsSource === 'external') {
						lyricsResult = await fetchLyricsFromAPI($player.currentTrack!);
						if (!lyricsResult) {
							lyricsResult = (await $client.getLyrics($player.currentTrack!)) || null;
			...
			} else if (isFullscreen && lyrics && currentTrackId === lastFetchedLyricsId) {
				scrollToCurrentLyric();
			}
		} else {
			currentTrackId = null;
	...
	async function toggleFullscreen() {
		if (isFullscreen) {
			...
			setTimeout(() => {
				...
				if (lyrics && $player.currentTrack && $player.currentTrack.id === lastFetchedLyricsId) {
					scrollToCurrentLyric();
				}
				updateCurrentLyric();
			}, 50);
	...
	function parseWordTimings(lines: SyncedLyric[]): EnhancedSyncedLyric[] {
		...
		const LAST_LINE_MIN_EFFECTIVE_DURATION = Math.max(DEFAULT_MEDIAN_LINE_DURATION, 2.0);
		...
		if (lines.length > 1) {
			for (let i = 0; i < lines.length - 1; i++) {
				...
				if (diff >= MEDIAN_FILTER_MIN_INTERVAL && diff <= MEDIAN_FILTER_MAX_INTERVAL) {
					lineIntervals.push(diff);
				}
			}
		}
		const medianSongLineDuration =
			lineIntervals.length > 0 ? calculateMedian(lineIntervals) : DEFAULT_MEDIAN_LINE_DURATION;
		...
		for (let i = 0; i < normalizedLines.length; i++) {
			...
			for (let j = 0; j < normalizedLines.length; j++) {
				if (i !== j) {
					const similarity = calculateStringSimilarity(normalizedLines[i], normalizedLines[j]);
		...
		function analyzeSingingPatterns(word: string): {
			...
			const isNoteTransitionWord =
				NOTE_TRANSITION_WORDS.includes(normalizedWord) || MELISMA_COMMON_WORDS.test(normalizedWord);
		...
		return lines.map((line, lineIndex) => {
			...
			const wordMatches = [
				...cleanText.matchAll(/([a-zA-Z0-9]+['']*[a-zA-Z0-9]*[.,!?;:]*)|([\p{P}]+)/gu)
			];
			...
			parentheticals.forEach((p) => {
				const isAlreadyCovered = wordsWithInfo.some(
					(w) => w.position >= p.startPos && w.position + w.word.length <= p.endPos
				);
				...
				if (isPartOfChorus) {
					const similarLineDurations = similarLines[lineIndex]
						.map((idx) => {
							...
							return nextTime ? nextTime - lines[idx].time : calculatedLineDuration;
						})
						.filter(
							(duration) =>
								duration >= MEDIAN_FILTER_MIN_INTERVAL && duration <= MEDIAN_FILTER_MAX_INTERVAL
						);
			...
			if (
				...
				calculatedLineDuration > medianSongLineDuration * LONG_PAUSE_MULTIPLIER_THRESHOLD
			) {
				effectiveLineDuration = medianSongLineDuration * CAPPED_DURATION_MULTIPLIER;
			} else if (isLastLine) {
				effectiveLineDuration = Math.max(medianSongLineDuration, LAST_LINE_MIN_EFFECTIVE_DURATION);
			}
			...
			const syllableCounts = words.map((word) => {
				if (word.startsWith('(') && word.endsWith(')')) {
					...
					return estimateSyllables(innerContent) * PARENTHETICAL_DURATION_MULTIPLIER;
				}
			...
			const isParentheticalWord = (word: string) => word.startsWith('(') && word.endsWith(')');

			const isComplexWord = (word: string) => {
				const cleanWord = word.replace(/[^\w]/g, '');
				return (
					cleanWord.length > COMPLEX_WORD_LENGTH_THRESHOLD || RARE_CHAR_PATTERN.test(cleanWord)
				);
			};
			...
			words.forEach((word, i) => {
				...
				if (hasPunctuation(word)) {
					...
					wordDuration *=
						PUNCTUATION_DURATION_MULTIPLIER[punctType] || PUNCTUATION_DURATION_MULTIPLIER.default;
				}
				...
				wordDuration = Math.max(
					MIN_WORD_DURATION,
					Math.min(wordDuration, effectiveLineDuration * MAX_WORD_DURATION_LINE_RATIO)
				);
				...
				if (i === words.length - 1) {
					...
					finalEndTime = Math.max(finalEndTime, currentTime + MIN_WORD_DURATION);
				...
				if (i < words.length - 1) {
					const actualDurationForThisWord = Math.max(0.01, finalEndTime - currentTime);
					...
					advance = Math.max(advance, actualDurationForThisWord * MIN_ADVANCE_RATIO_AFTER_OVERLAP);

					currentTime += advance;
					currentTime = Math.min(currentTime, lineEffectiveEndTime - MIN_WORD_DURATION);
				}
			});
	...
	async function fetchLyricsFromAPI(track: any): Promise<{
		...
		try {
			...
			const response = await fetch(`https://api.vmohammad.dev/lyrics?${params}`);

			if (!response.ok) {
				return null;
			}

			const enhancedLines: EnhancedSyncedLyric[] = (await response.json()).enhancedLyrics;

			if (!enhancedLines || !Array.isArray(enhancedLines) || enhancedLines.length === 0) {
				return null;
			}
	...
	function updateCurrentWord() {
		if (!lyrics?.enhanced || currentLyricIndex === -1 || !lyrics.enhancedLines) return;
		...
		const newWordIndex = currentLine.words.findIndex((word, i) => {
			const endTime = word.endTime || (currentLine.words[i + 1]?.time ?? Infinity);
...
{#if $player.currentTrack}
	...
	<div
		...
		} left-0 right-0 ${isFullscreen ? 'backdrop-blur-2xl' : 'backdrop-blur-md border-t'}`}
		in:slide={{
				...
				>
					...
					<div class="flex-1 flex gap-4 min-h-0">
						...
						>
							...
							<div class="flex-1 overflow-y-auto p-4 scrollbar-none" bind:this={lyricsContainer}>
								{#if lyrics}
									{#if lyrics.synced}
                                    ...
                                    {wordIndex <= currentWordIndex ? 'text-primary' : ''} 
                                    {wordIndex === currentWordIndex
                                       ...
                                       {i < currentLyricIndex ? 'opacity-40' : 'opacity-60'} 
					...
					<div class="flex items-center gap-4 min-w-0">
						{#if $player.currentTrack}
							...
							<div class="min-w-0 flex flex-col">
								<div class="flex items-center gap-2">
									<a class="font-medium truncate" href="/songs/{$player.currentTrack.id}"
								...
								<a
									...
									href="/artists/{$player.currentTrack.artistId}">{$player.currentTrack.artist}</a
								>
							</div>
						{/if}
					</div>
